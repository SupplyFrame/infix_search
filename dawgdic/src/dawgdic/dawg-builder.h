 ...
 private:
  ...
  bool InsertKey(const CharType *key, SizeType length, ValueType value) {
    ...
    for ( ; key_pos <= length; ++key_pos) {
      ...
      printf("Child_index at key pos %d is %d\n",(int)key_pos,(int)child_index);
      if (!child_index) {
        printf("Not child index, breaking\n");
        break;
      }
      ...
      printf("Key label is %c and unit label for child %d is %c\n",key_label,child_index,unit_label);

      // Checks the order of keys.
      if (key_label < unit_label) {
        printf("Error! Key label is less than unit label\n");
        return false;
      } else if (key_label > unit_label) {
        printf("Key label is greater than unit. Setting unit pool for child %d to true\n",child_index);
    ...
    for ( ; key_pos <= length; ++key_pos) {
      ...
      printf("Allocating a new child index child index %d\n",child_index);

      if (!unit_pool_[index].child()) {
        printf("Unit pool at %d is not child, setting state true at child %d\n",index,child_index);
        unit_pool_[child_index].set_is_state(true);
      }
      printf("Setting sibling of child %d as %d\n",child_index,unit_pool_[index].child());
      unit_pool_[child_index].set_sibling(unit_pool_[index].child());
      printf("Setting label of child %d as %c\n",child_index,key_label);
      unit_pool_[child_index].set_label(key_label);
      printf("Setting child of %d as %d\n",index,child_index);
      unit_pool_[index].set_child(child_index);
      printf("Pushing child index %d into unfixed\n",child_index);
  ...
  void FixUnits(BaseType index) {
    ...
    while (unfixed_units_.top() != index) {
      ...
      if (matched_index != 0) {
        ...
        for (BaseType i = unfixed_index; i != 0; i = unit_pool_[i].sibling()) {
          printf("Transition index is now %d\n",transition_index);
          base_pool_[transition_index].set_base(unit_pool_[i].base());
          printf("At base pool, setting base to %d\n",unit_pool_[i].base());
          label_pool_[transition_index] = unit_pool_[i].label();
          printf("At label pool, setting label to %c\n",unit_pool_[i].label());
          --transition_index;
        }
        matched_index = transition_index + 1;
        hash_table_[hash_id] = matched_index;
        printf("Setting hash table at %d to matched index %d\n",hash_id,matched_index);
        ++num_of_states_;
      }
      ...
      for (BaseType current = unfixed_index, next;
        ...
        printf("Freeing sibling %d\n",current);
        next = unit_pool_[current].sibling();
        FreeUnit(current);
      }

      printf("Setting child at unit pool %d to %d\n",unfixed_units_.top(),matched_index);
  ...
  bool AreEqual(BaseType unit_index, BaseType transition_index) const {
    ...
    for (BaseType i = unit_pool_[unit_index].sibling(); i != 0;
      ...
      printf("AreEqual: at transition index %d\n",transition_index);
    ...
    if (base_pool_[transition_index].has_sibling() == true) {
      printf("At transition %d has sibling so returning false\n",transition_index);
      return false;
    }
    ...
    for (BaseType i = unit_index; i;
         ...
         printf("Sibling %d Comparing base %d to %d and label %c to %c\n",i,unit_pool_[i].base(),base_pool_[transition_index].base(),unit_pool_[i].label(),label_pool_[transition_index]);
      if (unit_pool_[i].base() != base_pool_[transition_index].base() ||
          unit_pool_[i].label() != label_pool_[transition_index]) {
          printf("Not equal so returning false\n");
        return false;
      }
    }
    return true;
